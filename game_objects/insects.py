...
class BeeSwarm:
    def __init__(self, screen_width, screen_height, num_bees=10, swarm_radius=50, speed=2, scare_distance=100):
    ...
    def update(self, player_position, scaring_bees):
        # Check if the player pressed space to scare the swarm
        if scaring_bees:
            self.scared = True
        ...
        if self.scared:
            ...
            if direction.length() > self.swarm_radius:
                direction.normalize_ip()
    ...
    def render(self, screen):
        # Draw each bee as a small yellow circle
        for bee_position in self.bees:
            pygame.draw.circle(screen, (255, 255, 0), (int(bee_position.x), int(bee_position.y)), 3)
